#!/usr/bin/env python3
"""Scrape Simply Charlotte Mason products from simplycharlottemason.com (WooCommerce API)."""
...
API_BASE = "https://simplycharlottemason.com/wp-json/wc/store/products"
PER_PAGE = 100
SITE_NAME = "Simply Charlotte Mason"

CATEGORY_GRADES = {
    "kindergarten": "Kindergarten",
    "high-school": "High School",
    "legacy-curriculum": "Legacy Curriculum (All Ages)",
}

FORM_GRADES = {
    "form 1": "Form 1 (Grades 1-3)",
    "form 2": "Form 2 (Grades 4-6)",
    "form 3": "Form 3 (Grades 7-9)",
    "form 4": "Form 4 (Grades 10-12)",
    "form i": "Form 1 (Grades 1-3)",
    "form ii": "Form 2 (Grades 4-6)",
    "form iii": "Form 3 (Grades 7-9)",
    "form iv": "Form 4 (Grades 10-12)",
}
...
def format_price(prices: dict) -> str:
    ...
    if sale == "Free" and regular and regular != "Free":
        return f"Free (was {regular})"
...
def extract_ages(title: str, description: str, categories: list[dict]) -> str:
    ...
    for category in categories:
        ...
        if slug in CATEGORY_GRADES:
            label = CATEGORY_GRADES[slug]
            if label not in seen:
                seen.add(label)
                labels.append(label)

    for key, label in FORM_GRADES.items():
    ...
    if not labels:
        labels.append("Charlotte Mason; All Ages")
...
def main() -> int:
    ...
    print("Fetching Simply Charlotte Mason products...", file=sys.stderr)
    ...
    csv_path = data_dir / "simply-charlotte-mason-scraped.csv"
    json_path = data_dir / "simply-charlotte-mason-scraped.json"
