...
export function writeshopRowToSeedInput(row: WriteshopCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["writing"],
    description: [row.description?.trim(), `WriteShop resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "WriteShop homeschool resource.",
  };
}
