...
export type BreakingTheBarrierCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function breakingTheBarrierRowToSeedInput(row: BreakingTheBarrierCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Breaking the Barrier resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Breaking the Barrier homeschool resource.",
  };
}
