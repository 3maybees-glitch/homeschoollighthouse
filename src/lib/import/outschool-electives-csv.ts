...
export type OutschoolElectivesCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function outschoolElectivesRowToSeedInput(row: OutschoolElectivesCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ['electives'],
    description: [row.description?.trim(), `Outschool resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Outschool homeschool resource.",
  };
}
