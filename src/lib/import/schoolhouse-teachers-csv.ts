...
export type SchoolhouseTeachersCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function schoolhouseTeachersRowToSeedInput(row: SchoolhouseTeachersCsvRow) {
  ...
  return {
    ...
    religions: ['christian'],
    subjects: ["electives"],
    description: [row.description?.trim(), `Schoolhouse Teachers resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Schoolhouse Teachers homeschool resource.",
  };
}
