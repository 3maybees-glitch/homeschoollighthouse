...
export type EssentialsInWritingCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function essentialsInWritingRowToSeedInput(row: EssentialsInWritingCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["writing"],
    description: [row.description?.trim(), `Essentials in Writing resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Essentials in Writing homeschool resource.",
  };
}
