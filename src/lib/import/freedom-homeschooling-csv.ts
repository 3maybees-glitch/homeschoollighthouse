import type { ListingFormat, PriceType } from "@/types/listing";
import { parseAgeRange, parsePrices } from "@/lib/import/thsm-csv";
import { inferListingTypeWithHubCheck } from "@/lib/import/resource-hub";

export type FreedomHomeschoolingCsvRow = {
  title: string;
  website_url: string;
  source: string;
  grades_or_ages: string;
  prices_mentioned: string;
  description: string;
};

export function freedomHomeschoolingRowToSeedInput(row: FreedomHomeschoolingCsvRow) {
  const { priceType, priceMin, priceMax } = parsePrices(row.prices_mentioned);
  const { ageMin, ageMax } = parseAgeRange(row.grades_or_ages);
  const resolvedPriceType = (row.prices_mentioned ? priceType : "free") as PriceType;

  return {
    title: row.title.replace(/™/g, "").trim(),
    listingType: inferListingTypeWithHubCheck(row.website_url, "supplement"),
    format: "online" as ListingFormat,
    priceType: resolvedPriceType,
    priceMin,
    priceMax,
    websiteUrl: row.website_url,
    ageMin,
    ageMax,
    philosophies: ['eclectic'],
    religions: ['secular'],
    subjects: ["electives"],
    description: [row.description?.trim(), `Freedom Homeschooling resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Freedom Homeschooling homeschool resource.",
  };
}
