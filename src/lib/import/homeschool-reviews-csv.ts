...
export type HomeschoolReviewsCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function homeschoolReviewsRowToSeedInput(row: HomeschoolReviewsCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["electives"],
    description: [row.description?.trim(), `HomeschoolReviews.com resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "HomeschoolReviews.com homeschool resource.",
  };
}
