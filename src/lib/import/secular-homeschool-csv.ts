...
export type SecularHomeschoolCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function secularHomeschoolRowToSeedInput(row: SecularHomeschoolCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["electives"],
    description: [row.description?.trim(), `Secular Homeschool resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Secular Homeschool homeschool resource.",
  };
}
