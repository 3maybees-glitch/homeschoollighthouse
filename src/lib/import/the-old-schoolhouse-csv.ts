...
export type TheOldSchoolhouseCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function theOldSchoolhouseRowToSeedInput(row: TheOldSchoolhouseCsvRow) {
  ...
  return {
    ...
    religions: ['christian'],
    subjects: ["electives"],
    description: [row.description?.trim(), `The Old Schoolhouse Store resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "The Old Schoolhouse Store homeschool resource.",
  };
}
