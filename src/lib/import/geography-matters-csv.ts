...
export type GeographyMattersCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function geographyMattersRowToSeedInput(row: GeographyMattersCsvRow) {
  ...
  return {
    ...
    ageMin,
    ageMax,
    philosophies: ['unit_studies'],
    religions: ['secular'],
    subjects: ['geography'],
    description: [row.description?.trim(), `Geography Matters resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Geography Matters homeschool resource.",
  };
}
