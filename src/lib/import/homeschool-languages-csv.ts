...
export type HomeschoolLanguagesCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function homeschoolLanguagesRowToSeedInput(row: HomeschoolLanguagesCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["foreign_language"],
    description: [row.description?.trim(), `Homeschool Languages resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Homeschool Languages homeschool resource.",
  };
}
