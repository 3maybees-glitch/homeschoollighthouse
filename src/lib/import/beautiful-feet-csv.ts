...
export type BeautifulFeetCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function beautifulFeetRowToSeedInput(row: BeautifulFeetCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Beautiful Feet Books resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Beautiful Feet Books homeschool resource.",
  };
}
