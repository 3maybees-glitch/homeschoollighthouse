...
export type RosettaStoneCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function rosettaStoneRowToSeedInput(row: RosettaStoneCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Rosetta Stone resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Rosetta Stone homeschool resource.",
  };
}
