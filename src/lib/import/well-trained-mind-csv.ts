...
export type WellTrainedMindCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function wellTrainedMindRowToSeedInput(row: WellTrainedMindCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["electives"],
    description: [row.description?.trim(), `Well-Trained Mind resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Well-Trained Mind homeschool resource.",
  };
}
