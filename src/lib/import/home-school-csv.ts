...
export function homeSchoolRowToSeedInput(row: HomeSchoolCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ["electives"],
    description: [row.description?.trim(), `Homeschool World resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Homeschool World homeschool resource.",
  };
}
