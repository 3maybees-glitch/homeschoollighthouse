...
export function canonPressRowToSeedInput(row: CanonPressCsvRow) {
  ...
  return {
    ...
    religions: ['christian'],
    subjects: ['electives'],
    description: [row.description?.trim(), `Canon Press resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Canon Press homeschool resource.",
  };
}
