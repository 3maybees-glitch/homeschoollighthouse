...
export type SpellingPowerCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function spellingPowerRowToSeedInput(row: SpellingPowerCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Spelling Power resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Spelling Power homeschool resource.",
  };
}
