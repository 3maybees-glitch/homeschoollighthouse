...
export type CalicoSpanishCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function calicoSpanishRowToSeedInput(row: CalicoSpanishCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Calico Spanish resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Calico Spanish homeschool resource.",
  };
}
