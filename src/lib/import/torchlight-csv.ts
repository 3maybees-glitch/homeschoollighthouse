...
export function torchlightRowToSeedInput(row: TorchlightCsvRow) {
  ...
  return {
    ...
    ageMin,
    ageMax,
    philosophies: ['secular', 'charlotte_mason'],
    religions: ['secular'],
    subjects: ['reading', 'history'],
    description: [row.description?.trim(), `Torchlight Curriculum resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Torchlight Curriculum homeschool resource.",
  };
}
