...
export type SpellingYouSeeCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function spellingYouSeeRowToSeedInput(row: SpellingYouSeeCsvRow) {
  ...
  return {
    ...
    description: [row.description?.trim(), `Spelling You See resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Spelling You See homeschool resource.",
  };
}
