...
export type BluestockingPressCsvRow = {
  title: string;
  website_url: string;
  source: string;
...
export function bluestockingPressRowToSeedInput(row: BluestockingPressCsvRow) {
  ...
  return {
    ...
    religions: ['secular'],
    subjects: ['history', 'electives'],
    description: [row.description?.trim(), `Bluestocking Press resource: ${row.website_url}`]
      .filter(Boolean)
      .join(" "),
    shortDescription:
      row.description?.slice(0, 120) || "Bluestocking Press homeschool resource.",
  };
}
